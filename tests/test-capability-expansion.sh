...
for skill in "${new_skills[@]}"; do
  ...
  for section in "When to use" "Workflow" "Output" "Guardrails"; do
    awk -v heading="## $section" '{ sub(/\r$/, "") } $0 == heading { found=1 } END { exit !found }' "$file" || fail "$skill is missing $section"
  done
