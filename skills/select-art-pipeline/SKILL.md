...
description: Resolve CLOUD, LOCAL, CSS3, or CODE for a visual task from a task-local prompt override or STYLE.md, record selection evidence, and stop on failure instead of switching pipelines. Use for visual implementation, generated imagery, CSS/SVG art, or authored Three.js model work.
---
...
- A prompt explicitly requests image generation, local ComfyUI, CSS/vector art, or code-first Three.js models.
...
2. Normalize explicit prompt language to `CLOUD`, `LOCAL`, `CSS3`, or `CODE`. A prompt override may select any registered pipeline, including one outside the contract's allowed alternatives.
...
6. For `CLOUD` or `LOCAL` image generation, also read `MEDIA.md`. `CSS3` and `CODE` do not require `MEDIA.md`.
